...
:apply_changes
    ...
    :: Modify configuration file (enable root)
    attrib -R "%CONF_FILE%"
    copy "%CONF_FILE%" "%TEMP_FILE%" /Y >nul
    powershell -Command "(Get-Content '%TEMP_FILE%') -replace 'bst.instance.%version%.enable_root_access=\"0\"', 'bst.instance.%version%.enable_root_access=\"1\"' -replace 'bst.feature.rooting=\"0\"', 'bst.feature.rooting=\"1\"' | Set-Content '%TEMP_FILE%'"
...
:undo_all_changes_final
    ...
    :: Notify success and clean up
    powershell -Command "Write-Host 'Changes undone successfully for %version%.' -ForegroundColor Green"
    powershell -Command "Remove-MpPreference -ExclusionPath '%~dp0' 2>$nul" && (
        echo Exclusion removed for path: %~dp0
    )
    pause
