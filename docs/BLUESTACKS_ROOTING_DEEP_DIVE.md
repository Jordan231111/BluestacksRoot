...
### 4.3 The fix (proven)
Populate `/data/adb/magisk` with the **canonical 64‑bit install set**, copied byte‑for‑byte from the matching APK (`Magisk-27.2-kitsune-4.apk`, `lib/$ABI` renamed + `assets/*.sh`):
```
...
**DONE & PROVEN (cold boot, emulator root OFF):** Magisk v27.2‑kitsune‑4 is the **sole working root** on
...
HD‑Player patch intact; backups retained.
